          ...
          <div className="md:w-10/12 w-full mx-auto mt-4">
            <div className="space-y-4">
              {faqData?.filter((item) => item.type == 1)?.map((item, idx) => (
                <div key={idx}>
                  <div>
                    <p className="font-extrabold text-[#777] min-[1400px]:text-base text-sm mb-[30px]">
                      Q- {item.question}
                    </p>
                  </div>
              ...
              {faqData?.filter((item) => item.type == 2)?.map((item, idx) => (
                <div key={idx}>
                  <div>
                    <p className="font-montserrat font-extrabold text-[#777] min-[1400px]:text-base text-[15px] ">
                      {item.question}
                    </p>
                  </div>
                  <div className="flex text-[#7b7b7b] min-[1400px]:text-base text-sm pt-1">
